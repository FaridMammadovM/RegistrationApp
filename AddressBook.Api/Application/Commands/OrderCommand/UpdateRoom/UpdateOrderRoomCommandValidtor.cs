...
{
    public class UpdateOrderRoomCommandValidtor : AbstractValidator<UpdateOrderRoomCommand>
    {
        public UpdateOrderRoomCommandValidtor()
        {
            RuleFor(v => v.Id).NotNull().NotEmpty().WithMessage("Id cannot be null")
                .GreaterThan(0).WithMessage("Id should be greater than 0");
            RuleFor(v => v.UpdateBy).NotNull().NotEmpty().WithMessage("InsertBy cannot be null")
